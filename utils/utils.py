...
from einops import rearrange  # 如不需要可移除
from skorch import NeuralNetClassifier
from sklearn.model_selection import train_test_split
from sklearn.metrics import classification_report, accuracy_score

from collections.abc import Mapping, Sequence  # ✅ Py3.11 兼容
...
def starting_logs(data_type, da_method, exp_log_dir, src_id, tgt_id, run_id):
    """初始化日志目录并返回 logger"""
    log_dir = os.path.join(exp_log_dir, f"{src_id}_to_{tgt_id}_run_{run_id}")
...
def to_device(input_obj, device):
    """将输入数据移动到指定设备（CPU或GPU）——Py3.11 兼容"""
    if torch.is_tensor(input_obj):
        return input_obj.to(device=device)
    elif isinstance(input_obj, str):
        return input_obj
    elif isinstance(input_obj, Mapping):
        return {k: to_device(v, device=device) for k, v in input_obj.items()}
    elif isinstance(input_obj, Sequence) and not isinstance(input_obj, (str, bytes)):
        return [to_device(v, device=device) for v in input_obj]
    else:
...
def copy_Files(destination):
    ...
    copy("configs/data_model_configs.py", os.path.join(destination_dir, "data_model_configs.py"))
    copy("configs/hparams.py", os.path.join(destination_dir, "hparams.py"))
    copy("configs/sweep_params.py", os.path.join(destination_dir, "sweep_params.py"))
...
def get_weight_gpu(source_feature, target_feature, validation_feature, configs, device):
    ...
    source_feature = _copy.deepcopy(source_feature.detach().cpu())
    target_feature = _copy.deepcopy(target_feature.detach().cpu())
    ...
    feature_for_train, feature_for_test, label_for_train, label_for_test = train_test_split(
        all_feature, all_label, train_size=0.8)
...
# ---------------------- EATA Memory / 选择器 ----------------------
class EATAMemory:
    def __init__(self, maxlen: int = 4096, device: str | torch.device = "cpu"):
        self.maxlen = maxlen
        self.device = torch.device(device)
        self._feats: torch.Tensor | None = None    # [M, D], unit-normalized
        self._probs: torch.Tensor | None = None    # [M, C]
    ...
    def density(self, feats: torch.Tensor, K: int = 5):
        ...
        # ✅ 若当前批次不在 memory.device，上移一次缓存，避免 cuda/cpu 混用；并确保 dtype 对齐
        if self._feats.device != feats.device:
            self.to(feats.device)

        feats = F.normalize(feats, dim=1).to(self._feats.dtype)
        sims = feats @ self._feats.T                     # [B, M]
        k_eff = min(K, sims.size(1))
        topk, _ = torch.topk(sims, k=k_eff, dim=1)       # [B, k_eff]
        dens = topk.mean(dim=1)
        stats = {
...
def select_eata_indices(
    ...
    # —— 新增更稳控制项 —— #
    e_margin_min: float = 0.38,            # 熵阈地板
    balance_per_class: bool = True,        # 类均衡选样
    min_per_class: int = 2,                # 每类保底
    fuse_density: bool = True,             # 与密度融合排序
    density_high_is_good: bool = False     # dens 越小越近(=距离)；若为相似度设 True
):
    """返回: (idx[<=B], log_str)"""
    ...
    if balance_per_class:
        ...
        for c in range(C):
            ...
            if fuse_density and (k_eff > 0):
                ...
                order = torch.argsort(ent[idx_c])  # 低熵优先
